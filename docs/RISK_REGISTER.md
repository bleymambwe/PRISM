...
| R12 | PRISM's polynomial-time behavior is landscape-conditional: mismatched operators hard-fail on plateau-rich landscapes and deceptive landscapes defeat all tested operators. | Overclaiming "O(n³ log n) runtime" without conditions would be scientifically wrong. | Open (documented 2026-07-06) | Frame the runtime bound as conditional in all papers; pursue operator-portfolio PRISM to reduce operator-choice risk. |
